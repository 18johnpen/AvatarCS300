# Unified R script to process all categories
...
BrainWaveAnalysis <- R6Class("BrainWaveAnalysis",
  public = list(
    ...
    action = NULL,               # Action category (e.g., takeoff, right, etc.)
    
    # Constructor to initialize the object with files
    initialize = function(file_path, action) {
      self$action <- action
      # Load the data
    ...
    plot_data = function() {
      ...
      p <- ggplot(self$df_subset, aes(x = sample_index, y = Amplitude)) +
        ...
        theme_minimal() +
        labs(
          title = paste("Brain Wave Signals:", self$action),
          x = "Number of Samples",
...
# Iterate over all categories and process data
categories <- c("takeoff", "right", "left", "land", "forward", "backward")
base_path <- "/Avatar/plots/plot_waves/brainwaves-csv"

for (action in categories) {
  cat("Processing category:", action, "\\n")
  action_path <- file.path(base_path, action)
  analysis <- BrainWaveAnalysis$new(action_path, action)
}
